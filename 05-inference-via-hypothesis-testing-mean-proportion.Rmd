...
In this equation, $x_i$ is a 1 or a 0 depending on whether or not we detected the $i$th projectile, and $n$ is the total number of observations we have. We can use this notation to state the research problem we describe above more formally:  We want to determine if $\pi > 0.8 = 80%$. To do this, we're going to use our estimat, $\hat \pi$. 

The simplest approach is to just say that $\hat \pi$ is our best guess for $\pi$, so let's just ask if $\hat \pi> 0.8$. Based on our data above, we have:  $\hat\pi = $`r phat`. This is larger than 0.8, so it looks like we're good to go! Unforunately, 
